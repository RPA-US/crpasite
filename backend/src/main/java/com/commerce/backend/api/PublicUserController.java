...
public class PublicUserController extends ApiController {
    ...
    public ResponseEntity registerUserAccount(@RequestBody @Valid UserForm userForm, BindingResult bindingResult, HttpServletRequest request) {
    ...
    public ResponseEntity forgotPasswordPut(@RequestBody @Valid PasswordForgotForm passwordForgotForm, BindingResult bindingResult, HttpServletRequest request) {
        if (bindingResult.hasErrors()) {
            System.out.println("eee");
            return new ResponseEntity(HttpStatus.BAD_REQUEST);
        }
        tokenService.validateForgotPassword(passwordForgotForm);
        return new ResponseEntity(HttpStatus.OK);
    }
